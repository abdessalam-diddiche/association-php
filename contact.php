					...
					<form class="form-area " id="myForm" action="" method="POST" class="contact-form text-right">
						<div class="row">
							<div class="col-lg-6 form-group">
								<input name="nom" placeholder="Enter your name" onfocus="this.placeholder = ''" onblur="this.placeholder = 'Enter your name'"
							...
							<div class="col-lg-6 form-group">
								<textarea class="common-textarea form-control" name="message" placeholder="Messege" onfocus="this.placeholder = ''"
								 onblur="this.placeholder = 'Messege'" required="" type="text"></textarea>
							</div>
							<div class="col-lg-12 d-flex justify-content-between">
								<div class="form-group form-button" style="text-align: left;"></div>
								<input class="form-submit" type="submit" name="send" style="float: right;" value="Send message">
							</div>
						</div>
					</form>
				</div>
...
if(isset($_POST['send'])){
	  ...
	  if($nom==""){
		  $error[$nom]="il faut entrer le nom";
	  }
	  else if(!ctype_alpha($nom)){
		  $error[$nom]="utiliser seulement les caractères!";
	  }
	  else if(strlen($nom>30)){
		  $error[$nom]="le nom est trops large!";
	  }

	  if($email==""){
		  $error[$email]="il faut entrer l'email";
	  }
	  if($subject==""){
		  $error[$subject]="il faut entrer le sujet";
	  }
	  if($message==""){
		  $error[$message]="il faut entrer un message";
	  }
	...
	*/
	  ...
	  $sql = "INSERT INTO contact (nom,email,subject,message) VALUE (?,?,?,?)";
	  $stmtinsert = $db->prepare($sql);
	  $result = $stmtinsert->execute([$nom,$email,$subject,$message]);
